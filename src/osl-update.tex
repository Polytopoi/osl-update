...
It is legal for there to be different subexpression links with the same output; this entails that that subexpression's value can validly be determined based on more than one step type and/or more than one input vector. So that each output subexpression value is uniquely defined by the logic circuit inputs, it should be that in any case, all links that can be instantiated for an output $o$ result in the same value of $o$.
