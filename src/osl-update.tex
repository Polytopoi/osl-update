...
OSL is a layer of abstraction over $\Sigma^1_1$ formulas. $\Sigma^1_1$ formulas are a sublanguage of bounded second order arithmetic \cite{buss} extended with $\text{ind}_<$ and max built-in functions, defined as follows:
...
\begin{enumerate}
	...
	\item The strong prenex normal form gets compiled into a logic circuit. A logic circuit is a structure similar to a Halo 2 \cite{halo2-book,halo2-github} circuit, with some key differences, as follows. Logic circuits do not have top-level lookup arguments, but instead just gate constraints and equality constraints. Instead of taking the form $p = 0$, for $p$ a polynomial, gate constraints are quantifier-free logic formulas which can assert equalities and inequalities over terms involving variables, constants, addition, multiplication, the max and $\text{ind}_<$ functions, and lookups.
...
\begin{thebibliography}{6}
	...
	\bibitem{buss} Samuel Buss. \textit{Bounded Arithmetic.} 1985. Doctoral dissertation, Princeton University. \url{https://mathweb.ucsd.edu/~sbuss/ResearchWeb/BAthesis/Buss\_Thesis\_OCR.pdf}

	\bibitem{sequent-calculus} Edward Z. Yang. \textit{Logitext Tutorial.} Accessed May 2022. \url{http://logitext.mit.edu/logitext.fcgi/tutorial}
